...
writeBytes gdb address bytes = do
  ...
  withLanguage gdb "c" $ do
    -- We can probably be much more efficient here, but I fear endianness issues.
    forM_ (zip indicesI bytes) $ \(n, byte) -> do
      let
        byteHex = "0x" <> showHex (fromIntegral @_ @Int byte) ""
        addressHex = "0x" <> showHex (address + fromIntegral n) ""
      runCommand gdb [i|set *(unsigned char *)#{addressHex} = #{byteHex}|]
