...
use bittide_hal::manual_additions::index::IndexTy;
use bittide_hal::switch_c::ValidEntry;
...
fn test_result(result: &str) -> ! {
    ...
    writeln!(uart, "RESULT: {}", result).unwrap();
    loop {}
}
...
fn expect<T: core::fmt::Debug + PartialEq>(msg: &str, expected: T, actual: T) {
    if expected != actual {
        ...
        // } else {
        //     writeln!(INSTANCES.uart, "PASS: {} | {:?}", msg, expected).unwrap();
    }
}

#[cfg_attr(not(test), entry)]
fn main() -> ! {
    let active_entry0: [Index![17]; 16] =
        core::array::from_fn(|i| unsafe { IndexTy::new_unchecked(i as u8) });
    ...
    let cal_shadow: [ValidEntry; 16] = core::array::from_fn(|i| hal::types::ValidEntry {
