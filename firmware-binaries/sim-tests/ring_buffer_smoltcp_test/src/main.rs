...
fn main() -> ! {
    ...
    for _ in 0..100 {
        ...
        if !command_sent
            && link_manager
                .try_send(&CommandWire::from(Command::RequestUgnReport))
                .is_ok()
        {
    ...
    for _ in 0..500 {
        ...
        if subordinate_sent_edges < subordinate_report.count as usize {
            if let Some(edge) = subordinate_report.edges[subordinate_sent_edges] {
                if link_subordinate.try_send(&UgnEdgeWire::from(edge)).is_ok() {
        ...
        if manager_received_edges < manager_count as usize {
            if let Ok(edge) = link_manager.try_recv::<UgnEdgeWire>().map(UgnEdge::from) {
