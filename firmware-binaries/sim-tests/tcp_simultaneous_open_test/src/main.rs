...
//! Simulation test for TCP simultaneous open over ring_buffer links.
//!
//! A single CPU drives both endpoints of a cross-wired pair of ring_buffers
//! (link0 transmits on the buffer link1 receives from, and vice versa). Both
//! endpoints actively connect to each other at the same time, exercising the
//! TCP simultaneous open path of [`LinkInterface`]. After the connection is
//! established, the test exchanges raw bytes (`try_recv_bytes`) and typed
//! messages (`try_recv`) in both directions, then closes both links
//! gracefully.
//!
//! # Usage
//!
//! This binary is loaded by `simResultTcpOpen` in
//! `Bittide.Instances.Tests.RingBuffer` (`bittide-instances:unittests`), which
//! simulates it and inspects the UART output. On success the test prints
//! `=== Simultaneous Open Success ===`; the simulation reads until
//! `=== Test Complete ===`.
#![no_std]
...
/// hold the largest message exchanged in this test.
...
fn main() -> ! {
    ...
    let mut tcp_rx_buffer0 = [0u8; TCP_BUFFER_SIZE];
    let mut tcp_tx_buffer0 = [0u8; TCP_BUFFER_SIZE];
    let mut socket_storage0 = [SocketStorage::EMPTY; 1];
    ...
    let mut tcp_rx_buffer1 = [0u8; TCP_BUFFER_SIZE];
    let mut tcp_tx_buffer1 = [0u8; TCP_BUFFER_SIZE];
    let mut socket_storage1 = [SocketStorage::EMPTY; 1];
    ...
    // Poll until both links are established. All this manual polling (here and
    // below) highlights the need to switch to `async` (e.g. `embassy`), see
    // https://github.com/bittide/bittide-hardware/issues/1116.
    for _ in 0..10000 {
        link0.poll();
        link1.poll();
    ...
    if !link0.is_established() || !link1.is_established() {
        ...
        uwriteln!(uart, "[ERROR] === Test Failed ===").unwrap();
