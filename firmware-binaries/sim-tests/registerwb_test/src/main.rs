...
use bittide_hal::{
    hals::register_wb as hal,
    manual_additions::{bitvector::BitVector, index::Index, signed::Signed, unsigned::Unsigned},
    types,
};
...
fn main() -> ! {
    ...
    {
        let v0_ref = [16, 32, 64, 128, 3, 9, 27, 81].map(|n| BitVector::new([n]).unwrap());
