...
use bittide_hal::manual_additions::aligned::Aligned4;
...
fn main() -> ! {
    ...
    for (i, &word) in test_data.iter().enumerate() {
        buffer.set_data(i, word);
    }

    for (i, &expected) in test_data.iter().enumerate() {
        expect("word-based access failed", Some(expected), buffer.data(i));
    }
    ...
    // Test 2: Aligned4 byte slices
    uwriteln!(uart, "Test 4-byte aligned slices of various sizes").unwrap();

    for size in 1..=8 {
        uwriteln!(uart, "  Testing size: {}", size).unwrap();
        // Write slice of test_data
        buffer.write_slice(&test_data.get()[0..size], 0);

        // Read back and verify
        let mut verify = Aligned4::new([0u8; 32]);
        buffer.read_slice(&mut verify.get_mut()[0..size], 0);

        for (i, &expected) in test_data.get().iter().enumerate().take(size) {
            expect("4-byte aligned slices failed", expected, verify.get()[i]);
        }
    ...
    uwriteln!(uart, "Test unaligned slices of various sizes").unwrap();
    for alignment in 1..=3 {
        for size in 1..=4 {
            for offset in 1..=3 {
                ...
                buffer.write_slice(&test_data.get()[alignment..alignment + size], offset);

                for i in 0..size {
                    let expected = test_data.get()[alignment + i];
                    let actual = unsafe { buffer.as_slice() }[offset + i];
                    expect("Unaligned slices failed", expected, actual);
                }
