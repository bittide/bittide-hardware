...
void c_main(void) {
    ...
    gather_unit_write_slice(&gather, source, 0, MEM_SIZE);

    // Metacycle 2: Write second half to gather memory
    gather_unit_wait_for_new_metacycle(&gather);
    gather_unit_write_slice(&gather, &source[MEM_SIZE], 0, MEM_SIZE);

    // Metacycle 3: Read first half from scatter memory
    scatter_unit_wait_for_new_metacycle(&scatter);
    scatter_unit_read_slice(&scatter, destination, 0, MEM_SIZE);

    // Metacycle 4: Read second half from scatter memory
    scatter_unit_wait_for_new_metacycle(&scatter);
    scatter_unit_read_slice(&scatter, &destination[MEM_SIZE], 0, MEM_SIZE);
