...
fn main() -> ! {
    ...
    // Wait for the clocks to be a bit more stable before starting UGN capture. For more
    ...
    while !link_startups.iter().all(|ls| ls.is_done()) {
        ...
        // Delay the all_stable signal by counting iterations. This way the elastic buffers
