...
// "hals/soft_ugn_demo_management_unit/device_instances.h" followed by the
// bittide_* headers.
...
/**
 * @brief Align ring_buffers across all ports using a per-port state machine.
 *
 * This function implements a distributed alignment protocol that aligns
 * the receive ring_buffers on each port with the partner's transmit
 ...
 *     * If found at offset N > 0: call `set_clear_at_count(N)` to
