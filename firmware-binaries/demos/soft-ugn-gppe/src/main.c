...
#define STARTING_TICK_SND_OFFSET 2000
#define STARTING_TICK_REC_OFFSET 3550
...
int c_main(void) {
    ...
    for (uint32_t k = 0; k < NUM_PERIODS; k++) {
        ...
        if (wait_result == WAIT_ALREADY_PASSED) {
            uint32_t expected_mc = (uint32_t)(metacycle_start / METACYCLE_CLOCKS);
            uint64_t current_cycles = timer_now_cycles(&peripherals.timer);
            uint32_t current_mc = (uint32_t)(current_cycles / METACYCLE_CLOCKS);
            PRINT_MISSED_METACYCLE(&peripherals.uart, expected_mc, current_mc);
        }
    ...
    uart_putdec(&peripherals.uart, ugn_ctx.node_id);
    uart_puts(&peripherals.uart, "\n");
    uart_puts(&peripherals.uart, "Number of ports: ");
    uart_putdec(&peripherals.uart, ugn_ctx.num_ports);
    uart_puts(&peripherals.uart, "\n");
    uart_puts(&peripherals.uart, "Incoming UGNs known: ");
    uart_putdec(&peripherals.uart, ugn_ctx.number_incoming_link_ugns_known);
    uart_puts(&peripherals.uart, "/");
    uart_putdec(&peripherals.uart, ugn_ctx.num_ports);
    uart_puts(&peripherals.uart, "\n");
    uart_puts(&peripherals.uart, "Outgoing UGNs acknowledged: ");
    uart_putdec(&peripherals.uart, ugn_ctx.number_outgoing_link_ugns_acknowledged);
    uart_puts(&peripherals.uart, "/");
    uart_putdec(&peripherals.uart, ugn_ctx.num_ports);
    uart_puts(&peripherals.uart, "\n\n");

    // Print incoming link UGNs
    uart_puts(&peripherals.uart, "Incoming Link UGNs:\n");
    for (uint32_t i = 0; i < ugn_ctx.num_ports; i++) {
        UgnEdge* edge = &ugn_ctx.incoming_link_ugn_list[i];
        if (edge->is_valid) {
            uart_puts(&peripherals.uart, "  Port ");
            uart_putdec(&peripherals.uart, i);
            uart_puts(&peripherals.uart, ": src_node=");
            uart_putdec(&peripherals.uart, edge->src_node);
            uart_puts(&peripherals.uart, ", src_port=");
            uart_putdec(&peripherals.uart, edge->src_port);
            uart_puts(&peripherals.uart, ", dst_node=");
            uart_putdec(&peripherals.uart, edge->dst_node);
            uart_puts(&peripherals.uart, ", dst_port=");
            uart_putdec(&peripherals.uart, edge->dst_port);
            uart_puts(&peripherals.uart, ", ugn=");
            uart_putdec(&peripherals.uart, (uint32_t)edge->ugn);
            uart_puts(&peripherals.uart, "\n");
        } else {
            uart_puts(&peripherals.uart, "  Port ");
            uart_putdec(&peripherals.uart, i);
            uart_puts(&peripherals.uart, ": (not valid)\n");
        }
    }

    // Print outgoing link UGNs
    uart_puts(&peripherals.uart, "\nOutgoing Link UGNs:\n");
    for (uint32_t i = 0; i < ugn_ctx.num_ports; i++) {
        UgnEdge* edge = &ugn_ctx.outgoing_link_ugn_list[i];
        if (edge->is_valid) {
            uart_puts(&peripherals.uart, "  Port ");
            uart_putdec(&peripherals.uart, i);
            uart_puts(&peripherals.uart, ": src_node=");
