...
#define RECEIVE_SPACING (3 * BUFFER_SIZE) // Spacing between RECEIVE events
#define RECEIVE_PERIOD (RECEIVE_SPACING * NUM_PORTS)
...
int c_main(void) {
  ...
  for (uint32_t port = 0; port < NUM_PORTS; port++) {
    uint64_t send_time =
        start_cycles_write + port * SEND_SPACING - outgoing_offsets[port];
  ...
  for (k = 0; k < NUM_PERIODS; k++) {
    ...
    if (current_event.type == EVENT_TYPE_SEND) {
      ...
      if (ugn_ctx.outgoing_link_ugn_list[current_event.port].is_valid) {
        msg_type = MSG_TYPE_ACKNOWLEDGE;
      } else {
        // Reschedule if we dont have the outgoing ugn yet
