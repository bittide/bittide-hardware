...
// timer
#define PRINT_MISSED_METACYCLE(PERIPHERALS_PTR, METACYCLE_START_CYCLES,        \
                               METACYCLE_CLOCKS)                               \
  do {                                                                         \
...
// Message for starting cycle information - takes scatter unit and calculates
// values
#define PRINT_START_INFO(PERIPHERALS_PTR, METACYCLE_CLOCKS)                    \
  do {                                                                         \
    uint32_t start_mc =                                                        \
        scatter_unit_metacycle_count(&((PERIPHERALS_PTR)->scatter_units[0])) + \
        1;                                                                     \
...
#define PRINT_QUEUE_STATS(PERIPHERALS_PTR, EVENT_QUEUE_PTR)                    \
  do {                                                                         \
    ...
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((EVENT_QUEUE_PTR)->size));                          \
    uart_puts(&((PERIPHERALS_PTR)->uart), "/");                                \
...
#define PRINT_COMPLETION_STATS(PERIPHERALS_PTR, ITERATIONS, START_CYCLES,      \
                               METACYCLE_CLOCKS)                               \
  do {                                                                         \
...
// metacycle_clocks
#define PRINT_FIRST_EVENT_INFO(PERIPHERALS_PTR, EVENT_STRUCT,                  \
                               METACYCLE_CLOCKS)                               \
  do {                                                                         \
    ...
    if ((EVENT_STRUCT).type == EVENT_TYPE_SEND) {                              \
      ...
      if ((EVENT_STRUCT).msg_type == MSG_TYPE_ANNOUNCE) {                      \
        uart_puts(&((PERIPHERALS_PTR)->uart), "ANNOUNCE");                     \
      } else {                                                                 \
        uart_puts(&((PERIPHERALS_PTR)->uart), "ACKNOWLEDGE");                  \
      }                                                                        \
      uart_puts(&((PERIPHERALS_PTR)->uart), ")\n");                            \
...
#define PRINT_LOOP_CONFIG(PERIPHERALS_PTR, MAX_ITERATIONS, SEND_PERIOD,        \
                          RECEIVE_PERIOD, METACYCLE_CLOCKS)                    \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\nLoop configuration:\n");          \
...
#define PRINT_PROTOCOL_GOALS(PERIPHERALS_PTR, UGN_CTX_PTR)                     \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\nProtocol goals:\n");              \
    uart_puts(&((PERIPHERALS_PTR)->uart),                                      \
              "  Discover incoming UGNs for all ");                            \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->num_ports));                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), " ports\n");                         \
    uart_puts(&((PERIPHERALS_PTR)->uart),                                      \
              "  Acknowledge outgoing UGNs for all ");                         \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->num_ports));                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), " ports\n");                         \
  } while (0)
...
#define PRINT_INIT_INFO(PERIPHERALS_PTR, UGN_CTX_PTR, BUFFER_SIZE,        \
                        SEND_PERIOD, RECEIVE_PERIOD, NUM_PORTS)                \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart),                                      \
              "========================================\n");                   \
    ...
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->node_id));                           \
    uart_puts(&((PERIPHERALS_PTR)->uart), " | Ports: ");                       \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->num_ports));                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), " | Buffer size: ");                 \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(BUFFER_SIZE));          \
    uart_puts(&((PERIPHERALS_PTR)->uart), " cycles\n");                        \
...
#define PRINT_DEADLINE_STATS(PERIPHERALS_PTR, UGN_CTX_PTR)                     \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\nDeadline Statistics:\n");         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "--------------------\n");           \
    uart_puts(&((PERIPHERALS_PTR)->uart), "SEND events:\n");                   \
    uart_puts(&((PERIPHERALS_PTR)->uart), "  Met deadlines: ");                \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->met_send_count));             \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Missed deadlines: ");           \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->missed_send_count));          \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
    uart_puts(&((PERIPHERALS_PTR)->uart), "RECEIVE events:\n");                \
    uart_puts(&((PERIPHERALS_PTR)->uart), "  Met deadlines: ");                \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->met_receive_count));          \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Missed deadlines: ");           \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->missed_receive_count));       \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
    uart_puts(&((PERIPHERALS_PTR)->uart), "INVALIDATE events:\n");             \
    uart_puts(&((PERIPHERALS_PTR)->uart), "  Met deadlines: ");                \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->met_invalidate_count));       \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Missed deadlines: ");           \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)((UGN_CTX_PTR)->missed_invalidate_count));    \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
    uart_puts(&((PERIPHERALS_PTR)->uart), "Total:\n");                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "  Met deadlines: ");                \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->met_send_count + (UGN_CTX_PTR)->met_receive_count + (UGN_CTX_PTR)->met_invalidate_count));    \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Missed deadlines: ");           \
    uart_putdec(                                                               \
        &((PERIPHERALS_PTR)->uart),                                            \
        (uint64_t)((UGN_CTX_PTR)->missed_send_count + (UGN_CTX_PTR)->missed_receive_count + (UGN_CTX_PTR)->missed_invalidate_count));   \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
  } while (0)
    ...
    for (uint32_t port = 0; port < (UGN_CTX_PTR)->num_ports; port++) {         \
      ...
      if (incoming->is_valid && outgoing->is_valid) {                          \
        ...
        if (!incoming->is_valid)                                               \
          uart_puts(&((PERIPHERALS_PTR)->uart), "incoming");                   \
        if (!incoming->is_valid && !outgoing->is_valid)                        \
          uart_puts(&((PERIPHERALS_PTR)->uart), " and ");                      \
        if (!outgoing->is_valid)                                               \
          uart_puts(&((PERIPHERALS_PTR)->uart), "outgoing");                   \
        uart_puts(&((PERIPHERALS_PTR)->uart), ")\n");                          \
      }                                                                        \
    }                                                                          \
  } while (0)
...
#define PRINT_FOUND_INCOMING_UGN(PERIPHERALS_PTR, EVENT_TIME, PORT, UGN)       \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "I P");                              \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(PORT));                 \
    uart_puts(&((PERIPHERALS_PTR)->uart), ": ");                               \
    uart_putdec_signed(&((PERIPHERALS_PTR)->uart), (UGN));                     \
    uart_puts(&((PERIPHERALS_PTR)->uart), " @ ");                              \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(EVENT_TIME));           \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
  } while (0)

// Message for printing found outgoing UGN
#define PRINT_FOUND_OUTGOING_UGN(PERIPHERALS_PTR, EVENT_TIME, PORT, UGN)       \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "O P");                              \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(PORT));                 \
    uart_puts(&((PERIPHERALS_PTR)->uart), ": ");                               \
    uart_putdec_signed(&((PERIPHERALS_PTR)->uart), (UGN));                     \
    uart_puts(&((PERIPHERALS_PTR)->uart), " @ ");                              \
...
#define PRINT_FINAL_SEND_SCHEDULED(PERIPHERALS_PTR, PORT, TIME)                \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "FS P");                             \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(PORT));                 \
    uart_puts(&((PERIPHERALS_PTR)->uart), " @ ");                              \
    uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(TIME));                 \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
  } while (0)

// Message for printing statistics regarding what kind of messages were sent and
// received
#define PRINT_MESSAGE_COUNT_STATES(PERIPHERALS_PTR, UGN_CTX_PTR)        \
  do {                                                                         \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\nMessage Counts:\n");             \
    uart_puts(&((PERIPHERALS_PTR)->uart), "  Received announces: ");                \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->incoming_announce_count));               \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Received acknowledges: ");          \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->incoming_acknowledge_count));            \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Announces sent: ");         \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->outgoing_announce_count));           \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n  Acknowledges sent: ");      \
    uart_putdec(&((PERIPHERALS_PTR)->uart),                                    \
                (uint64_t)((UGN_CTX_PTR)->outgoing_acknowledge_count));        \
    uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                               \
  } while (0);

// Assertion macro to check that SEND event_time is correctly aligned
#define ASSERT_SEND_TIME_VALID(PERIPHERALS_PTR, event_time, offset)            \
  do {                                                                         \
    uint64_t expected_time =                                                   \
        ((event_time / METACYCLE_CLOCKS) * METACYCLE_CLOCKS) + (offset)        \
    if ((event_time) != expected_time) {                                       \
      uart_puts(&((PERIPHERALS_PTR)->uart),                                    \
                "[UGN WARNING] SEND event_time misaligned: ");                 \
      uart_putdec(&((PERIPHERALS_PTR)->uart), event_time);                     \
      uart_puts(&((PERIPHERALS_PTR)->uart), " (expected ");                    \
      uart_putdec(&((PERIPHERALS_PTR)->uart), expected_time);                  \
      uart_puts(&((PERIPHERALS_PTR)->uart), ")\n");                            \
    }                                                                          \
  } while (0)

// Assertion macro to check that SEND and RECEIVE periods are coprime
#define ASSERT_PERIODS_COPRIME(PERIPHERALS_PTR, SEND_PERIOD, RECEIVE_PERIOD)   \
  do {                                                                         \
    ...
    if (a != 1) {                                                              \
      ...
      uart_putdec(&((PERIPHERALS_PTR)->uart), (SEND_PERIOD));                  \
      uart_puts(&((PERIPHERALS_PTR)->uart), " and ");                          \
      uart_putdec(&((PERIPHERALS_PTR)->uart), (RECEIVE_PERIOD));               \
      uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                             \
    }                                                                          \
  } while (0)

#define PRINT_DEADLINE_MISSED(PERIPHERALS_PTR, EVENT_PTR, EVENT_TIME) \
    do {                                                                       \
        uart_puts(&((PERIPHERALS_PTR)->uart),                              \
                  "[UGN WARNING] Event missed deadline: type=");           \
        uart_puts(&((PERIPHERALS_PTR)->uart),show_event_type((EVENT_PTR)->type));\
        uart_puts(&((PERIPHERALS_PTR)->uart), ", scheduled_time=");        \
        uart_putdec(&((PERIPHERALS_PTR)->uart), (uint64_t)(EVENT_TIME));   \
        uart_puts(&((PERIPHERALS_PTR)->uart), ", now=");               \
        uint64_t now_cycles = timer_now_cycles(&((PERIPHERALS_PTR)->timer)); \
        uart_putdec(&((PERIPHERALS_PTR)->uart), now_cycles);              \
        uart_puts(&((PERIPHERALS_PTR)->uart), "\n");                       \
    } while (0);
