...
pub fn generate_type_desc<'ir>(
    ...
    let variants = if let Some(variants) = varis.variants_by_type.get(&desc.name) {
        ...
        // generate_type_definition(ctx, varis, None, &mut refs, desc)
        quote! {
            // compile_error!("Type without a monomorph variant");
        }
    };
...
fn generate_repr<'ir>(ctx: &'ir IrCtx, desc: &'ir TypeDescription) -> TokenStream {
    match &desc.definition {
        TypeDefinition::DataType {
            names: _,
            constructors,
...
pub fn generate_type_ref(
    ...
    match &ctx.type_refs[handle] {
        TypeRef::BitVector(handle) => {
            ...
            // let size = generate_type_ref(ctx, varis, variant, refs, *handle);
            // quote! {
            //    BitVector<#size>
            // }
        }
