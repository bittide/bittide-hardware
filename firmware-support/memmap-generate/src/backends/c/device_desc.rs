    ...
    if let TypeRef::Vector(len, inner) = ty {
        ...
        for i in 0..*n {
            ...
            write_type_to_addr(
                ...
                &base,
                &dest,
                DestinationType::ArraySubscript,
                *inner,
                indent,
                code,
            );
        ...
        let dest_deref = match dest_ty {
            DestinationType::ArraySubscript => "",
            DestinationType::Pointer => &format!("*(volatile {ty_ref}*)"),
        };
        writeln!(
            code,
            "{indent}{dest_deref}{destination_var} = *(volatile {ty_ref}*){base_addr};"
        )
        .unwrap();
    }
}
