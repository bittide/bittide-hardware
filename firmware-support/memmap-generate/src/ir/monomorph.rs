    ...
    pub fn monomorph_type_descs(
        ...
        loop {
            ...
            for var in range.handles() {
                ...
                // next.extend(mono_subs.values().copied());
    ...
    fn monomorph_type_ref(
        ...
        match &self.ctx.type_refs[handle] {
            TypeRef::Reference { name, args } => {
                ...
                // let any_monomorph_args =
                //     arg_state.iter().any(|s| *s == ArgMonomorphState::Monomorph);

                // if !any_monomorph_args {
                //     return MonomorphTypeRefResult::RawArg(&self.mapping.type_refs[&handle]);
                // }
