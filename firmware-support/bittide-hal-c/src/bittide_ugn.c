...
bool send_ugn_to_port(UgnContext *ctx, Timer *timer, Event *event) {
  ...
  uint32_t offset = event->event_time % unit->memory_len;
  uint64_t deadline = event->event_time + unit->memory_len;
...
bool check_incoming_buffer(UgnContext *ctx, Timer *timer, Event *event) {
  ...
  bool message_available = ugn_read_message(&ctx->scatter_units[event->port], offset, &msg);
  CompareResult cmp_result = timer_compare_cycles(timer, deadline);
  ...
  bool valid_message = process_ugn_message(&msg, event->port, ctx->node_id, event->event_time,
                                           &edge_in, &edge_out);
  if (!valid_message) {
    return false;
  }
...
void invalidate_port(UgnContext *ctx, Timer *timer, Event *event) {
  ...
  uint64_t deadline = event->event_time + gather_unit->memory_len;
  uint32_t offset = event->event_time % ctx->gather_units[port].memory_len;
  if (port >= ctx->num_ports)
    return;
