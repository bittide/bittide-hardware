...
void dna_read(volatile uint8_t *dna_register, dna_t out) {
  ...
  //   Bytes 20-23: Padding
  const volatile uint32_t *maybe_dna_words =
      (const volatile uint32_t *)dna_register;
  ...
  while (1) {
    ...
    if (present == 1) {
      ...
      out[0] = maybe_dna_words[2]; // bits [31:0]   at bytes 8-11 (least
                                   // significant word)
      out[1] = maybe_dna_words[3]; // bits [63:32]  at bytes 12-15
      out[2] = maybe_dna_words[4]; // bits [95:64]  at bytes 16-19 (most
                                   // significant word)
      return;
    }
