...
// alignment_offset is baked into the receive event time (local_time),
// so no separate correction is needed here.
...
static inline bool send_ugn_to_port(UgnContext *ctx, Timer timer,
  ...
  transmit_ring_buffer_write_slice(*unit, buffer, 0, 6);
...
static inline bool check_incoming_buffer(UgnContext *ctx, Timer timer,
  ...
  timer_wait_until_cycles(timer, event->event_time);
  bool message_available = ugn_read_message(unit, 0, &msg);
  CompareResult cmp_result = timer_compare_cycles(timer, deadline);
...
// Invalidate old transmit buffer data for a specific port
static inline void invalidate_port(UgnContext *ctx, Timer timer, Event *event) {
  ...
  timer_wait_until_cycles(timer, event->event_time);
  transmit_ring_buffer_write_slice(*transmit_unit, zeros, 0, 6);
  CompareResult cmp_result = timer_compare_cycles(timer, deadline);
