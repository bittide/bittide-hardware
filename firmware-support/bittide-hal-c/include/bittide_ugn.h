...
static inline const char* show_event_type(EventType type) {
    switch (type) {
        case EVENT_TYPE_SEND: return "SEND";
        case EVENT_TYPE_RECEIVE: return "RECEIVE";
        case EVENT_TYPE_INVALIDATE: return "INVALIDATE";
...
static inline const char* show_msg_type(UgnMessageType type) {
    switch (type) {
        case MSG_TYPE_ANNOUNCE: return "ANNOUNCE";
        case MSG_TYPE_ACKNOWLEDGE: return "ACKNOWLEDGE";
        default: return "UNKNOWN";
    }
}
...
static inline Event make_send_event(UgnMessageType msg_type, uint64_t event_time, uint32_t port) {
