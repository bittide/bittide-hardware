...
typedef struct {
  // Receive and transmit ringbuffers (one per port)
  ReceiveRingbuffer *receive_ringbuffers;
  TransmitRingbuffer *transmit_ringbuffers;
  uint32_t num_ports;
...
static inline void
ugn_context_init(UgnContext *ctx, ReceiveRingbuffer *receive_ringbuffers,
                 TransmitRingbuffer *transmit_ringbuffers, uint32_t num_ports,
                 uint32_t node_id, UgnEdge *incoming_list,
                 UgnEdge *outgoing_list, uint32_t max_degree) {
  ctx->receive_ringbuffers = receive_ringbuffers;
  ctx->transmit_ringbuffers = transmit_ringbuffers;
...
static inline uint32_t ugn_calculate_receive_offset(const UgnContext *ctx,
  ...
  return (uint32_t)(event_time % RECEIVE_RINGBUFFER_DATA_LEN);
}
...
static inline uint32_t ugn_calculate_transmit_offset(const UgnContext *ctx,
  ...
  (void)ctx;
  (void)port; // Unused in this implementation
  return (uint32_t)(event_time % TRANSMIT_RINGBUFFER_DATA_LEN);
}
...
static inline bool ugn_read_message(ReceiveRingbuffer *unit, uint32_t offset,
                                    UgnMessage *msg) {
  uint64_t data[6];
  receive_ringbuffer_read_slice_wrapping(*unit, data, offset, 6);
...
static inline bool send_ugn_to_port(UgnContext *ctx, Timer timer,
                                    Event *event) {
  TransmitRingbuffer *unit = &ctx->transmit_ringbuffers[event->port];
  uint32_t port = event->port;
  uint32_t offset = ugn_calculate_transmit_offset(ctx, port, event->event_time);
  uint64_t deadline = event->event_time + TRANSMIT_RINGBUFFER_DATA_LEN;
  ...
  transmit_ringbuffer_write_slice_wrapping(*unit, buffer, offset, 6);
...
static inline bool check_incoming_buffer(UgnContext *ctx, Timer timer,
                                         Event *event) {
  ReceiveRingbuffer *unit = &ctx->receive_ringbuffers[event->port];
  ...
  uint64_t deadline = event->event_time + RECEIVE_RINGBUFFER_DATA_LEN;
...
static inline void invalidate_port(UgnContext *ctx, Timer timer, Event *event) {
  uint32_t port = event->port;
  TransmitRingbuffer *transmit_unit = &ctx->transmit_ringbuffers[port];
  uint64_t deadline = event->event_time + TRANSMIT_RINGBUFFER_DATA_LEN;
  uint32_t offset = ugn_calculate_transmit_offset(ctx, port, event->event_time);
  ...
  timer_wait_until_cycles(timer, event->event_time);
  transmit_ringbuffer_write_slice_wrapping(*transmit_unit, zeros, offset, 6);
  CompareResult cmp_result = timer_compare_cycles(timer, deadline);
