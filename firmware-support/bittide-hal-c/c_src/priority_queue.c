...
// Assigns the item with the highest priority to 'result' without removing it.
//
// pq: A pointer to the FixedIntPriorityQueue.
// result: result pointer to assign to (if queue not empty)
//
// Returns false if queue is empty
bool pq_peek_min(const FixedIntPriorityQueue *pq, PriorityQueueItem *result) {
  if (pq_is_empty(pq)) {
    return false;
  }
  *result = pq_unchecked_peek_min(pq);
  return true;
}
...
PriorityQueueItem pq_unchecked_peek_min(const FixedIntPriorityQueue *pq) {
  uint32_t min_index = find_min_index(pq);
