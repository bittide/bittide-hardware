...
void test_peek() {
  ...
  PriorityQueueItem peeked;
  assert(!pq_peek_min(&pq, &peeked)); // Should return false on empty queue

  pq_insert(&pq, 50, 100);
  assert(pq_peek_min(&pq, &peeked));
  assert(peeked.data == 50);
