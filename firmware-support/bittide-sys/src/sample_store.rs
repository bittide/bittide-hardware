...
use bittide_hal::manual_additions::bitvector::BitVector;
...
impl SampleStore {
    pub fn new(memory: SampleMemory, store_samples_every: usize) -> Self {
        ...
        // memory.set_data(0, BitVector::new(0u32.to_le_bytes()).unwrap());
        memory.set_data(0, unsafe { BitVector::new_unchecked([0; 4]) });
    ...
    fn do_store(
        ...
        self.memory.set_data(
            start_index,
            // BitVector::new(local_clock_lsbs.to_le_bytes()).unwrap(),
            unsafe { BitVector::new_unchecked(local_clock_lsbs.to_le_bytes()) },
        );
        self.memory.set_data(
            start_index + 1,
            // BitVector::new(local_clock_msbs.to_le_bytes()).unwrap(),
            unsafe { BitVector::new_unchecked(local_clock_msbs.to_le_bytes()) },
        );
        ...
        self.memory.set_data(
            start_index + 5,
            // BitVector::new((net_speed_change as u32).to_le_bytes()).unwrap(),
            unsafe { BitVector::new_unchecked((net_speed_change as u32).to_le_bytes()) },
        );
        ...
        if bump_counter && self.memory.data(start_index + WORDS_PER_SAMPLE).is_some() {
            self.memory.set_data(
                0,
                // BitVector::new(((n_samples_stored + 1) as u32).to_le_bytes()).unwrap(),
                unsafe { BitVector::new_unchecked(((n_samples_stored + 1) as u32).to_le_bytes()) },
            );
        }
    }
