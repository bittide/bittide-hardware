...
//! as ring_buffers for point-to-point communication. The ring_buffers must
...
//!
//! The receive ring_buffer is volatile - its contents can change at any time due to
//! incoming data. To safely handle this, we:
//! 1. Disable writes to the buffer so incoming frames can't change its contents while
//!    we process them. The ring buffer itself keeps spinning (its internal counter keeps
//!    incrementing to maintain alignment); only writes to the buffer memory are suppressed.
...
/// Upper bound on the MTU, matching the standard Ethernet MTU. Keeps packets a
/// sane size even when the ring_buffers are large.
...
/// Device implementation for ring_buffer communication.
///
/// Provides a simple interface for point-to-point IP communication using
/// scatter/gather units. The ring_buffers handle all internal state management.
///
...
{
    /// Create a new ring_buffer device.
    ///
    /// The ring_buffers must already be aligned using the alignment protocol.
    ...
    pub fn new(rx_buffer: AlignedReceiveBuffer<Rx, Tx>, tx_buffer: Tx) -> Self {
        ...
        let mtu = (Rx::DATA_LEN * 8).min(MAX_MTU).min(Tx::DATA_LEN * 8) - PACKET_HEADER_SIZE;
        assert!(rx_buffer.is_aligned(), "RX buffer is not aligned ");
        // The alignment procedure leaves the RX buffer disabled; enable it here.
        // From this point on the device owns the enable bit: `receive` expects the
        // buffer to be enabled between calls and panics otherwise.
        rx_buffer.buffer.set_enable(true);
        debug!(
    ...
    Tx: TransmitRingBufferInterface + 'static,
{
    ...
    fn receive(&mut self, _timestamp: Instant) -> Option<(Self::RxToken<'_>, Self::TxToken<'_>)> {
        // The buffer should always be enabled between calls to receive; finding it disabled
        // means a previous call failed to restore it, which would silently drop all traffic.
        if !(self.rx_buffer.buffer.get_enable()) {
            panic!("RX buffer is disabled at start of receive - this should not happen.");
        }
        // Disable writes to the RX buffer so incoming frames can't change its contents while
        // we process them. The ring buffer itself keeps spinning to maintain alignment.
        self.rx_buffer.buffer.set_enable(false);

        // Make sure loads and stores do not cross this boundary to ensure correct order of
        // hardware accesses.
        ...
        unsafe {
            // Read header fields directly from hardware buffer
            // Header format: CRC32 (4 bytes) + sequence (2 bytes) + length (2 bytes)
            let crc = u32::from_le((hw_buffer_ptr as *const u32).read_volatile());
            let seq_num = u16::from_le((hw_buffer_ptr.add(4) as *const u16).read_volatile());
            let packet_len =
                u16::from_le((hw_buffer_ptr.add(6) as *const u16).read_volatile()) as usize;

            // Make sure loads and stores do not cross this boundary to ensure correct order of
            // hardware accesses before returning.
            core::sync::atomic::fence(core::sync::atomic::Ordering::AcqRel);
            ...
            if packet_len < MIN_IP_PACKET_SIZE || packet_len > self.mtu {
                warn!(
                    "Invalid packet length: {packet_len} (must be {MIN_IP_PACKET_SIZE}-{})",
                    self.mtu
                );
                self.rx_buffer.buffer.set_enable(true);
                return None;
            ...
            // Create slice directly from hardware buffer for CRC validation. Writes to the
            // buffer are disabled at this point, so its contents are stable and plain
            // (non-volatile) reads are fine.
...
/// Receive token for ring_buffer device.
///
...
/// Transmit token for ring_buffer device
    ...
    Tx: TransmitRingBufferInterface,
{
        ...
        F: FnOnce(&mut [u8]) -> R,
    {
        ...
        // This points directly to the memory-mapped TX ring_buffer
        ...
        unsafe {
            ...
            trace!("tx packet len {len} total {total_len}");
