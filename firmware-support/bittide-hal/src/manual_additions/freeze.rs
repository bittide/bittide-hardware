// SPDX-FileCopyrightText: 2026 Google LLC
//
// SPDX-License-Identifier: Apache-2.0

use crate::manual_additions::signed::Signed;
use crate::manual_additions::unsigned::Unsigned;

pub trait FreezeInterface {
    const EB_COUNTERS_LEN: usize;

    fn freeze_counter(&self) -> Unsigned<32, u32>;
    fn local_clock_counter(&self) -> Unsigned<64, u64>;
    fn number_of_sync_pulses_seen(&self) -> Unsigned<32, u32>;
    fn cycles_since_sync_pulse(&self) -> Unsigned<32, u32>;

    fn eb_counters(&self, idx: usize) -> Option<Signed<32, i32>>;
    /// # Safety
    /// `idx` must be less than `EB_COUNTERS_LEN`.
    unsafe fn eb_counters_unchecked(&self, idx: usize) -> Signed<32, i32>;
    fn eb_counters_volatile_iter(&self) -> impl DoubleEndedIterator<Item = Signed<32, i32>> + '_;

    fn set_freeze(&self, val: ());
}

macro_rules! impl_freeze_interface {
    ($t:ty) => {
        impl FreezeInterface for $t {
            const EB_COUNTERS_LEN: usize = <$t>::EB_COUNTERS_LEN;

            fn freeze_counter(&self) -> Unsigned<32, u32> {
                <$t>::freeze_counter(self)
            }
            fn local_clock_counter(&self) -> Unsigned<64, u64> {
                <$t>::local_clock_counter(self)
            }
            fn number_of_sync_pulses_seen(&self) -> Unsigned<32, u32> {
                <$t>::number_of_sync_pulses_seen(self)
            }
            fn cycles_since_sync_pulse(&self) -> Unsigned<32, u32> {
                <$t>::cycles_since_sync_pulse(self)
            }

            fn eb_counters(&self, idx: usize) -> Option<Signed<32, i32>> {
                <$t>::eb_counters(self, idx)
            }
            unsafe fn eb_counters_unchecked(&self, idx: usize) -> Signed<32, i32> {
                <$t>::eb_counters_unchecked(self, idx)
            }
            fn eb_counters_volatile_iter(
                &self,
            ) -> impl DoubleEndedIterator<Item = Signed<32, i32>> + '_ {
                <$t>::eb_counters_volatile_iter(self)
            }

            fn set_freeze(&self, val: ()) {
                <$t>::set_freeze(self, val)
            }
        }
    };
}

impl_freeze_interface!(crate::shared_devices::Freeze);
