...
impl AddressableBuffer {
    ...
    /// Write a byte slice to the data memory.
    ///
    ...
    pub fn write_slice(&self, src: &[u8], offset: usize) {
        ...
        for (i, &byte) in src.iter().enumerate() {
            ...
            let mut word = unsafe { self.data_unchecked(word_index) };
            word[byte_in_word] = byte;
            unsafe { self.set_data_unchecked(word_index, word) };
        }
    }

    /// Read a byte slice from the data memory.
    ///
    ...
    pub fn read_slice(&self, dst: &mut [u8], offset: usize) {
        ...
        for (i, byte) in dst.iter_mut().enumerate() {
            ...
            let word = unsafe { self.data_unchecked(word_index) };
