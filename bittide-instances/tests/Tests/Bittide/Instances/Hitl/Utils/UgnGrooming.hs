...
-- * Property: grooming restores a freshly-booted mesh to a prior state
...
same counter model, then groom them onto @λ^safe = goldenUgns + ε@ with 'groomCorrection'.

Grooming returns a per-node relabel @q@ (distinct from the hardware's @bootDelay@) plus
small per-link frame insertions. With a strictly positive margin @ε@ the restore is always
feasible: the slack on link @a -> b@ is @(D_a - D_b) + ε - ebDev_{a->b}@ where
@D_i = goldenBootDelay_i - currentBootDelay_i@ and @ebDev@ is the (bounded) elastic-buffer
deviation. Over any directed cycle the @D@ terms telescope away, leaving @Σ(ε - ebDev) > 0@
(we keep @ebDev < ε@), so there is no negative cycle. We assert the restore is feasible,
every frame insertion is nonnegative, and the relabel-then-pad reconstructs @λ^safe@ on
every link: @λ_{a->b} + q_b - q_a + frames_{a->b} == λ^safe_{a->b}@.
-}
...
prop_relabelRestoresPriorBoot = H.property $ do
  ...
  let
    ...
    currentByKey = Map.fromList [((e.srcNode, e.dstNode), toInteger e.ugn) | e <- currentUgns]
    lambdaSafeByKey = Map.fromList [((e.srcNode, e.dstNode), toInteger e.ugn) | e <- lambdaSafe]

  case groomCorrection (ugnGraph currentUgns) (ugnGraph lambdaSafe) of
    Infeasible ns -> do
      H.annotateShow ns
      H.failure
    Feasible{correction = q, framesToInsert} -> do
      -- 'q' is the per-node relabel grooming computed; every frame correction is a
      -- nonnegative insertion.
      H.assert (all (\(_, _, f) -> f >= 0) framesToInsert)
      -- Relabel-then-pad reconstructs the stored reference UGNs on every link:
      --   λ_{a->b} + q_b - q_a + frames_{a->b} == λ^safe_{a->b}
      forM_ framesToInsert $ \(i, j, f) -> do
        let
          qOf n = Map.findWithDefault 0 n q
          lookupKey k m = Map.findWithDefault (error "edge missing") k m
        lookupKey (i, j) currentByKey + qOf j - qOf i + f === lookupKey (i, j) lambdaSafeByKey
