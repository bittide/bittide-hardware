    ...
    ( SomeNat (snatProxy -> (SNat :: SNat n))
      ...
      do
        ...
        let
          ...
          outputValues = fmap toInteger <$>
            sampleWithResetN @System @(DiffResult (Index (m + 1))) d1 depth
              (\clk rst _ -> overflowResistantDiff clk rst
                (fromList inputTriggerValues)
                (fromList inputCounterValues)
              )

          expectedOutputs = List.tail
            $ List.reverse $ snd $ List.foldl golden (Nothing, [])
          ...
          golden (Just (x, o), xs) (c, _) =
            let y = x + toInteger (c - o)
                out = if y > toInteger (maxBound :: Index (m + 1))
                      then TooLarge
                      else Difference y
             in (Just (y, c), out : xs)
