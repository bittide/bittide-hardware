...
import Bittide.Instances.Hitl.ClockBoardConfiguration (circuitFn)
...
createDomain vXilinxSystem{vPeriod = hzToPeriod 2e6, vName = "Basic2"}
...
dut ::
  Circuit () (CSignal Basic1 Bool, Df Basic1 (BitVector 8))
dut = circuit $ do
  mm <- ignoreMM
  jtagIn <- idleSource
  (uartTx, spiDone, (Fwd sclk, Fwd mosi, Fwd ss)) <-
    circuitFn clockGen (resetGenN d2) (clockGen @Basic2)
      -< (mm, (jtagIn, Fwd miso))
