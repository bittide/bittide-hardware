...
bringUp refClk refRst = withBittideByteOrder $ circuit $ \(bootMm, muMm, ccMm, jtag, gths) -> do
  (bootUartBytes, _spiDone, spi, bootTransceiverWb0) <-
    withRefClockResetEnable
      $ bootPe bootPeConfig
      -< (bootMm, bootJtag)
  bootTransceiverWb1 <- idC -< bootTransceiverWb0

  transceiverWb0 <-
    withRefClockResetEnable arbiterMm
      -< [muTransceiverWb, bootTransceiverWb1]
  transceiverWb1 <- idC -< transceiverWb0
  ...
  Fwd tOutputs <-
    Transceiver.transceiverPrbsNWb @Bittide @GthRx @Ext200 @Basic125 @GthTxS @GthRxS
      ...
      -< (transceiverWb1, gths, Fwd (bundle switchDataOutOrLocalCounters))
