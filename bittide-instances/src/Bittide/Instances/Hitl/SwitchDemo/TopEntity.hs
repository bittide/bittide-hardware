...
module Bittide.Instances.Hitl.SwitchDemo.TopEntity where

import Clash.Explicit.Prelude
import Protocols

import Bittide.Hitl (HitlTestCase (..), HitlTestGroup (..), paramForHwTargets)
...
import qualified Bittide.Instances.Hitl.SwitchDemo.Driver as Driver
...
switchDemoTest boardClkDiff refClkDiff rxs rxns rxps spiS2M jtagIn _uartRx syncIn =
 ...
 where
  ...
  ( (_bootMm, _muMm, _ccMm, jtagOut, (txs, txns, txps))
    , ( spiM2S
        , syncOut
        , uartTx
        , swFincFdecs
