...
import Bittide.Instances.Hitl.Utils.Ugn (UgnEdge (..), hardwareUgnEdges, indexToNodeId)
      ...
      Gdb.withGdbs (L.length targets) $ \managementUnitGdbs -> do
        ...
        brackets serialStarts (liftIO . snd) $ \(L.map fst -> serials) -> do
          ...
          let
            ...
            tResets =
              map (\off -> checkedFromIntegral (toInteger sharedBase + toInteger off)) relabelPlan.resetOffsets
          liftIO $ do
            putStrLn
          ...
          liftIO $ do
            ...
            mapConcurrently_
              ...
              (L.zip managementUnitGdbs (toList relabelPlan.corrections))
            ...
            T.tryWithTimeoutOn
              ...
              goDumpCcSamples
              $ forConcurrently_ serials
              $ \serial -> waitForLine serial "[MU] Corrections applied successfully"
