...
counter clk rst ena = let c = register clk rst (toEnable ena) 0 (c + 1) in c
...
expectCounter ::
  ...
  Signal dom Bool
  -- ^ Error
expectCounter clk rst = sticky clk rst . mealy clk rst enableGen go 0
 where
  go c (Just e) = (c + 1, c /= e)
 ...
 where
  ...
  transceivers =
    transceiverPrbsN
      ...
      defTransceiverOptions{debugIla=True, debugFpgaIndex=bitCoerce <$> fpgaIndex}
      TransceiverInputs
        { clock = sysClk
        , reset = gthAllReset
