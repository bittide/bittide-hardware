...
-}
gdbCheck :: BitVector 32 -> Integer -> Gdb -> VivadoM ExitCode
gdbCheck expectedBE addr gdb = do
  bytes <- Gdb.readBytes @4 gdb addr
  let
...
driver testName targets = do
  ...
  brackets openOcdStarts (liftIO . (.cleanup)) $ \initOcdsData -> do
    ...
    Gdb.withGdbs (L.length targets) $ \ccGdbs -> do
      ...
      let ccWAIAddr = lookForWhoAmI memoryMapCc
      liftIO $ putStrLn [i|Using address #{showHex32 ccWAIAddr}|]
      gdbExitCodes0 <- mapM (gdbCheck ccWhoAmID ccWAIAddr) ccGdbs
      ...
      Gdb.withGdbs (L.length targets) $ \muGdbs -> do
        ...
        let muWAIAddr = lookForWhoAmI memoryMapMu
        liftIO $ putStrLn [i|Using address #{showHex32 muWAIAddr}|]
        gdbExitCodes1 <- mapM (gdbCheck muWhoAmID muWAIAddr) muGdbs
        ...
        Gdb.withGdbs (L.length targets) $ \gppeGdbs -> do
          ...
          let peWAIAddr = lookForWhoAmI memoryMapGppe
          liftIO $ putStrLn [i|Using address #{showHex32 peWAIAddr}|]
          gdbExitCodes2 <- mapM (gdbCheck gppeWhoAmID peWAIAddr) gppeGdbs
