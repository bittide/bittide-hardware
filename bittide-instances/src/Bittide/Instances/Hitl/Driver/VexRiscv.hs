...
driverFunc _name targets = do
  ...
  exitCodes <-
    forM (L.zip [0 ..] targets)
      $ \(targetIndex, (hwT, deviceInfo)) -> handle (liftIO . catchError deviceInfo)
        $ do
          ...
          let
            ...
            adaptersConfig =
              GdbAdaptersConfig
                { usbDev = deviceInfo.usbAdapterLocation
          ...
          liftIO $ withGdbAdapters adaptersConfig $ \adapters -> do
            -- make sure gdb-adapters is started properly
            ...
            withPicocom picocomConfig $ \pico -> do
              ...
              Gdb.withGdb $ \gdb -> do
                ...
                Gdb.setFile gdb $ firmwareBinariesDir RiscV Debug </> "hello"
