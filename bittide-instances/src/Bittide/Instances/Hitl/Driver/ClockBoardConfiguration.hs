...
module Bittide.Instances.Hitl.Driver.ClockBoardConfiguration where
    ...
    brackets openOcdStarts (liftIO . (.cleanup)) $ \initOcdsData -> do
      ...
      Gdb.withGdbs (L.length targets) $ \gdbs -> do
        ...
        brackets picocomStarts (liftIO . snd) $ \(L.map fst -> picocoms) -> do
          ...
          liftIO
            $ T.tryWithTimeout T.PrintActionTime "Waiting for test success" 10_000_000
            $ forConcurrently_ picocoms
            $ \pico ->
              waitForLine pico.stdoutHandle "Test passed"
