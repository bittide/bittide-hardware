...
-}
parseFile :: (HasCallStack) => FilePath -> IO (Int, [Sample])
parseFile filePath = do
  ...
  let (nLinks, rest) =
        Get.runGet
          ...
          bytes
  pure (nLinks, parseWith nLinks rest)
...
parseDirectory dir = do
  ...
  samples <- forConcurrently C.indicesI $ \fpgaNr -> do
    if fromIntegral fpgaNr < Topology.size ccConf.topology
      then snd <$> parseFile (dir </> [I.i|cc-samples-#{fpgaNr}.bin|])
      else pure []

  pure (ccConf, samples)

-- | Worker function for 'toNamedRecord'
...
sampleHeader nLinks =
  Csv.header $
    ...
    , "settleds"
      ...
      ++ ["eb_counter_" <> Char8.pack (show i) | i <- [0 .. nLinks - 1]]
