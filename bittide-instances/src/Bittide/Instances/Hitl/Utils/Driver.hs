...
-- | A generic wrapper around 'System.Timeout.Lifted.timeout' that 'fail's if the timeout is hit.
tryWithTimeout ::
  forall m a.
...
Assumes that each target has a probe named 'probe_handshakes_done', which should be tied to an 'and'
reduction of the handshakes signals from the transceivers.
-}
