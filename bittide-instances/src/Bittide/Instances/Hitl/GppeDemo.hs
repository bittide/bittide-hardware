...
on the topic.
-}
module Bittide.Instances.Hitl.GppeDemo (
  gppeDemoDut,
  gppeDemoTest,
  memoryMapCc,
  memoryMapMu,
...
import Bittide.Instances.Hitl.Dut.GppeDemo
...
gppeDemoDut refClk refRst skyClk rxSims rxNs rxPs miso jtagIn syncIn =
...
gppeDemoTest boardClkDiff refClkDiff rxs rxns rxps miso jtagIn _uartRx syncIn =
  -- Replace 'seqX' with 'hwSeqX' to include ILAs in hardware
  seqX
    testIla
    ( txs
 ...
 where
  ...
  ( txs :: Gth.SimWires Bittide LinkCount
    ...
    ) = gppeDemoDut refClk testReset boardClk rxs rxns rxps miso jtagIn syncIn
  ...
  captureFlag =
    riseEvery
      ...
      (SNat @(PeriodToCycles Basic125 (Milliseconds 2)))
{-# OPAQUE gppeDemoTest #-}
makeTopEntity 'gppeDemoTest

tests :: HitlTestGroup
tests =
  HitlTestGroup
    { topEntity = 'gppeDemoTest
    , extraXdcFiles =
        [ "jtag" </> "config.xdc"
        , "jtag" </> "pmod1.xdc"
        , "uart" </> "pmod1.xdc"
