...
import Bittide.Hitl (HitlTests{-, hitlVio-})
...
linkConfigurationTest refClkDiff sysClkDiff syncIn rxns rxps miso =
 ...
 where
  ...
  -- Consider test done if enabled links have been up consistently for
  -- 50 seconds. This is just below the test timeout of 60 seconds, so
  -- transceivers have ~10 seconds to come online reliably. This
  -- should be plenty.
  endSuccess = trueFor (SNat @(Seconds 50)) sysClk testRst linkMaskMatch
