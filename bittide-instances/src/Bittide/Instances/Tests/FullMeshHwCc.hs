...
fullMeshRiscvCopyTest clk rst tck jtagEn callistoResult jtagIn dataCounts = (fInc, fDec, jtagOut)
 where
  ((), (CSignal (unbundle -> (fInc, fDec)), jtagOut)) = toSignals
    ( circuit $ do
      ([wbA, wbB], jtag) <-
        processingElement peConfig clk rst tck jtagEn
      fIncDecCallisto -< wbA
...
fullMeshHwCcTest refClkDiff sysClkDiff syncIn rxns rxps miso =
 ...
 where
  ...
  (riscvFinc, riscvFdec, _) =
    fullMeshRiscvCopyTest callistoClock callistoReset callistoClock enableGen callistoResult (pure $ JtagIn low low) dataCounts
