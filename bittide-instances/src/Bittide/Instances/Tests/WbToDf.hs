...
dut = withBittideByteOrder $ withClockResetEnable clockGen (resetGenN d2) enableGen $ circuit $ \mm -> do
  ...
  [(prefixSrc, srcBus), (prefixDf, dfBus), (prefixUart, uartBus)] <-
    processingElement @_ dumpVcd peConfig -< (mm, jtagIdle)
  constBwd 0b001 -< prefixSrc
  constBwd 0b110 -< prefixDf
  constBwd 0b101 -< prefixUart
  (uartTx, _uartStatus) <- uartInterfaceWb d16 d2 uartBytes -< (uartBus, uartRx)

  [refWb] <- deviceWbC "WbToDfReference" -< srcBus
  registerWbCI_ refCfg testValue -< (refWb, Fwd (pure Nothing))
