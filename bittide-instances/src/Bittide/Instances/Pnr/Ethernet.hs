...
import Clash.Prelude (withClockResetEnable, HiddenClockResetEnable)
...
vexRiscEthernet sysClk sysRst sgmiiPhyClk (jtagin, uartIn, sgmiiIn) =
 ...
 where
  ...
  anConf = pure def{cAcknowledge=True, cDuplexMode = FullDuplex, cLinkSpeed = Speed1000, cPhyLinkStatus = True}
...
{-# ANN vexRiscEthernetTop (Synthesize
...
-- | Convert a reset signal from one domain to another domain and insert a register.
-- This can be used to move a reset signal from a domain with a synchronous reset to
-- a domain with an asynchronous reset.
unsafeResetDesynchronizer ::
  forall domA domS .
  (KnownDomain domA, KnownDomain domS) =>
  Clock domS ->
  Reset domS ->
  Reset domA
unsafeResetDesynchronizer clkIn rstIn = rstOut
 where
  rstOut = unsafeFromActiveHigh
    $ unsafeCoerce
    $ register clkIn rstIn enableGen True
    $ unsafeToActiveHigh rstIn
