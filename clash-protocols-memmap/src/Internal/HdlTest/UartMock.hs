...
import Protocols (Circuit (..), toSignals, Protocol (..), CSignal, Df)
...
magicUart = Circuit go
 where
  ...
  go :: (((), Signal dom (WishboneM2S addrWidth 4 (BitVector 32))), ()) -> ((SimOnly MemoryMap, Signal dom (WishboneS2M (BitVector 32))), ())
...
someOtherCircuit = withName "MyCircuit" $ circuit $ \(mm, (master, rx)) -> do
  ...
  (tx, _status) <-
    ...
    moreRealUart' d16 d16 -< (prA, (mmA, (a, rx)))
...
interconnect = Circuit go
 where
  ...
  go :: ( ((), Signal dom (WishboneM2S addrWidth (Div (BitSize a + 7) 8) a))
