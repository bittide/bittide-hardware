...
delayWishboneC = Circuit go
 where
  ...
  go = mooreB mooreTransfer mooreOut (WaitingForManager @aw @n @a)
   where
    mooreTransfer ::
    ...
    mooreTransfer WaitingForManager (m2s, _s2m)
      | m2s.strobe && m2s.busCycle = WaitingForSubordinate m2s
    mooreTransfer (WaitingForSubordinate _) (_m2s, s2m)
      | s2m.acknowledge || s2m.err || s2m.retry = AcknowledgingManager s2m
