...
fn main() -> ! {
    ...
    wait_for_start(&mut scatter_unit);

    // create a two metacycle "offset" to the ping program
    for _ in 0..2 {
        gather_unit.wait_for_new_metacycle();
    }
    pong(&mut gather_unit, &mut scatter_unit);
...
fn pong(gu: &mut GatherUnit<FRAME_SIZE>, su: &mut ScatterUnit<FRAME_SIZE>) {
    let mut num_buf = [0; 8];
...
fn wait_for_start(su: &mut ScatterUnit<FRAME_SIZE>) {
    let mut num_buf = [0; 8];
    loop {
        su.read_frame_memory(&mut num_buf);
        let num = u64::from_le_bytes(num_buf);
