...
genSwitchCalendar = do
  ...
  case (TN.someNatVal links, TN.someNatVal depth) of
    (SomeNat l, SomeNat d) -> do
      let
       realDepth = fromIntegral depth + 1
       realLinks = fromIntegral links + 1
       ...
       linksRange = Range.singleton realLinks
       depthRange = Range.singleton realDepth
      ...
      pure (SwitchCalendar (snatProxy l) (snatProxy d) cal)
  ...
  case switchCal of
    SwitchCalendar l@SNat d@SNat cal -> do
      let
        depth = 1 + snatToNum d
        links = 1 + snatToNum l
      simLength <- forAll $ (\l0 -> l0*depth + 1) <$> Gen.enum 0 10
      ...
      let
        ...
        simOut1 = P.drop (simLength - outputLength) $ fmap toList simOut
      ...
      Set.fromList (catMaybes$ P.concat simOut1) === Set.fromList (catMaybes expectedOutput)
