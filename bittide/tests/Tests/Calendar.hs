...
tests =
  testGroup
    ...
    , testPropertyNamed
        "Reading shadow buffer with wishbone"
        "readShadowCalendar"
        readShadowCalendar
...
genBVCalendar calSize bitWidth validityBits = do
 ...
 where
  ...
  go s b v@SNat = do
    ...
    case ( compareSNat calNatBits d32
         , compareSNat d1 calNatBits
         , compareSNat d1 b
         ) of
      (SNatLE, SNatLE, SNatLE) -> do
        cal <-
          Gen.list (Range.singleton $ fromIntegral calSize)
            $ genValidEntry
              (SNat @validityBits)
      ...
      _ ->
        error
          $ "genIntCalendar: Constraints not satisfied: 1 <= "
          <> show calNatBits
          <> " <= 32."
...
readShadowCalendar = property $ do
  ...
  case (calA, calS) of
    (BVCalendar snatA bwA valA calA', BVCalendar snatS bwS valS calS') ->
      case ( sameNat (asProxy snatA) (asProxy snatS)
           , sameNat (asProxy bwA) (asProxy bwS)
           , sameNat (asProxy valA) (asProxy valS)
           ) of
  ...
  case bvCal of
    BVCalendar (calSize' :: SNat calDepth) _ _ cal -> do
      ...
      let
        ...
        writeAndSwitch (dep, dur) =
          P.take (succ dur)
            $ [wbWriteOp (newDepthAddr, fromIntegral dep), swapCall]
            <> P.repeat wbNothingM2S
        topEntity writePort =
          (\(_, m, _) -> m)
            $ withClockResetEnable
              clockGen
        ...
        expectedOut =
          P.take simLength
            $ P.concatMap
              (\(fromIntegral -> n) -> P.replicate n False <> [True])
...
directedWbDecoding (wbM2S : m2sRest) (_ : s2mRest) = out
 where
  ...
  out
    | foundBeginning = entry : directedWbDecoding remainingM2S remainingS2M
