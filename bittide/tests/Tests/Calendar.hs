...
readMetacycleCount = property $ do
  let
    entryGen = do
      repetitions <- Gen.integral $ Range.linear 25 50
  ...
  let
    ...
    dut =
      let
        ...
        ?busByteOrder = endian
       in
        withClockResetEnable clockGen resetGen enableGen $ circuit $ \wb -> do
  ...
  let
    ...
    model req@(Wb.Write addr _ _) resp state
      | addr == endOfMetacycleAddress && resp.acknowledge = Right (succ state)
      | otherwise = err state req resp
    model req@(Wb.Read addr _) resp state
      | addr == metacycleCountAddress && resp.acknowledge && resp.readData == state =
          Right state
      | otherwise = err state req resp
  ...
  withClockResetEnable @System clockGen resetGen enableGen
    $ Wb.wishbonePropWithModel
      defExpectOptions{eoSampleMax = 10_0000, eoStopAfterEmpty = Just 1000}
      model
      (unMemmap dut)
      gen
      0
