...
tests =
  testGroup
    "Tests.Link"
    [ testPropertyNamed
        "txUnit can be set to continuously transmit the preamble and sequence counter."
        "txSendSC"
        txSendSC
    , testPropertyNamed
        "rxUnit can be set to continuously detect the preamble and store the sequence counter."
        "rxSendSC"
        rxSendSC
    , testPropertyNamed
        "Integration test with txUnit sending a preamble and sequence counter to rxUnit."
        "integration"
        integration
    ]
...
txSendSC = property $ do
  ...
  case tc of
    ( TestConfig
        ...
        seqCountIn <-
          forAll
            $ Gen.list (Range.singleton (simLength + iterationDuration))
        ...
        let
          ...
          onFrames =
            L.take onTime $ preambleFrames
              L.++ L.concatMap
                ((L.++ preambleFrames) . valToFrames)
...
rxSendSC = property $ do
  ...
  case tc of
    ( TestConfig
        ...
        remoteSeqCounts <-
          forAll
            . Gen.list (Range.singleton (1 + iterations))
        ...
        let
          ...
          filterCond inp
            | isJust inp =
                let (x, y) = (fromJust inp, fromJust $ L.head preambleFrames) in (x .&. y) /= y
            | otherwise = True
          genFrame = Gen.maybe genDefinedBitVector
          onFrames =
            L.take onTime $ L.concatMap ((preambleFrames <>) . valToFrames) remoteSeqCounts
        offFrames0 <-
          forAll $ Gen.list (Range.singleton offTime0) $ Gen.filter filterCond genFrame
        ...
        let
          topEntity (unbundle -> (wbIn0, linkIn, localCounter)) =
            wcre
              configRxUnit
              bs
              aw
 ...
 where
  directedDecoding ::
    ...
    [WishboneM2S aw bs (Bytes bs)] ->
    [WishboneS2M (Bytes bs)] ->
    [a]
    ...
    ( SomeNat llProxy
      , TestConfig
        ...
        let
          ...
          genFrame = Gen.maybe genDefinedBitVector
        gatherFrames0 <-
          forAll . Gen.list (Range.singleton delayCycles) $ Gen.filter filterCond genFrame
