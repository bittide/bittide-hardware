...
import Test.Tasty.Hedgehog (testPropertyNamed, HedgehogTestLimit (HedgehogTestLimit))
...
-- that is much faster.
createDomain vSystem{vName="FreeSlow", vResetKind=Synchronous, vPeriod=hzToPeriod 10e3}
createDomain vSystem{vName="Free",     vResetKind=Synchronous, vPeriod=hzToPeriod 60e3}
createDomain vSystem{vName="FreeFast", vResetKind=Synchronous, vPeriod=hzToPeriod 120e3}
...
gthCoreMock
  ...
  _txCtrl
  _refClk
 = ( txSerial, txSerial, txClk, rxClk
   , rxWord, pack <$> txDone, pack <$> rxDone, pack <$> txActive
   , 0, 0, 0, 0
   )
 where
  rxWord =
...
dut ::
  ...
  , HasSynchronousReset freeA, HasDefinedInitialValues freeA
  , HasSynchronousReset freeB, HasDefinedInitialValues freeB
  ) =>
...
dutRandomized ::
  ...
  ( HasSynchronousReset txA, HasDefinedInitialValues txA
  , HasSynchronousReset txB, HasDefinedInitialValues txB
  , HasSynchronousReset free, HasDefinedInitialValues free
  ) =>
...
tests =
  ...
  --      execute them.
  testGroup "Transceiver"
    [ adjustOption (\_ -> HedgehogTestLimit (Just 100)) $
        testGroup "Slow tests"
        [ testPropertyNamed "prop_noPressure_A_A_FreeSlow" "prop_noPressure_A_A_FreeSlow" prop_noPressure_A_A_FreeSlow
        , testPropertyNamed "prop_noPressure_A_A_Free" "prop_noPressure_A_A_Free" prop_noPressure_A_A_Free
        , testPropertyNamed "prop_noPressure_A_A_FreeFast" "prop_noPressure_A_A_FreeFast" prop_noPressure_A_A_FreeFast
        , testPropertyNamed "prop_noPressure_A_B_Free" "prop_noPressure_A_B_Free" prop_noPressure_A_B_Free
        , testPropertyNamed "prop_noPressure_B_A_Free" "prop_noPressure_B_A_Free" prop_noPressure_B_A_Free
        ]

    , adjustOption (\_ -> HedgehogTestLimit (Just 10)) $
        testGroup "Very slow tests"
