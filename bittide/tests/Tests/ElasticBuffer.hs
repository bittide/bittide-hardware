...
case_xilinxElasticBufferMaxBound = do
  let
    command = fromList $ L.replicate 3 (Just Fill) <> L.repeat Nothing
...
case_xilinxElasticBufferMinBound = do
  let
    command = fromList $ L.replicate 256 (Just Fill) <> L.repeat Nothing
    wData = pure (0 :: Unsigned 8)
    underflows =
      sampleN
        1024
...
case_resettableXilinxElasticBufferEq = do
  ...
  assertBool
    "elastic buffer should remain stable mode after stabilizing once"
    (L.and ebStables)
  assertBool
