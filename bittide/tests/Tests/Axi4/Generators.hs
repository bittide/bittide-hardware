...
prop_genAxisM2S = property $ do
  ...
  case TN.someNatVal $ fromIntegral dataWidth of
    SomeNat (Proxy :: Proxy dataWidth) -> do
      ...
      byteTypes <- forAll $ Gen.choice $ fmap pure $ allowedByteTypes
      ...
      mapM (\ byte -> cover 25 (H.LabelName $ "One or more " <> show byte) (isJust $ elemIndex byte axiBytes)) byteTypes
      mapM (\ byte -> cover 10 (H.LabelName $ "All " <> show byte) (all (== byte) axiBytes)) byteTypes
      mapM (\ byte -> cover 10 (H.LabelName $ "No " <> show byte) (all (/= byte) axiBytes)) byteTypes
      assert (all (`L.elem` byteTypes) $ toList axiBytes)
      assert (L.elem (_tlast axi) lastValues)
