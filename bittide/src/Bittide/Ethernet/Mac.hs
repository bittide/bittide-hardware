...
ethMac1GFifoC ::
  ...
  Signal rx Bool ->
  Circuit
    (Axi4Stream sys ('Axi4StreamConfig 1 0 0) Bool, CSignal rx GmiiRx)
    (Axi4Stream sys ('Axi4StreamConfig 1 0 0) Bool, CSignal tx GmiiTx, CSignal sys EthMacStatus)
...
ethMac1GFifo txFifoDepth rxFifoDepth sysClk sysRst txClk txRst rxClk rxRst miiSel txClkEna
 ...
 where
  ...
  gmiiRxData' = gmiiRxData <$> rxGmii
  gmiiRxValid' = gmiiRxValid <$> rxGmii
  gmiiRxError' = gmiiRxError <$> rxGmii

  -- Instantiate the blackbox
  gmiiTx = GmiiTx <$> gmiiTxData' <*> gmiiTxEnable' <*> gmiiTxError'
    ...
    InlineYamlPrimitive [Verilog] [__i|
      BlackBox:
        ...
        template: |-
          ...
          wire ~GENSYM[rxAxiData][#{rxAxiData}];
          ...
          wire ~GENSYM[gmiiTxData][#{gmiiTxData}];
          ...
          (
            ...
            .rx_axis_tdata(~SYM[#{gmiiRxData}]),
            .rx_axis_tkeep(~SYM[#{gmiiRxValid}]),
            .rx_axis_tvalid(~SYM[#{gmiiRxError}]),
