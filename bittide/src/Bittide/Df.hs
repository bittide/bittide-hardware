...
asciiDebugMux sTimeout labels0 = circuit $ \dfs -> do
  ...
  -- because we don't expect to generate any /abort/ transfers and the packet
  -- boundaries are used internally in 'asciiDebugMux' only.
  unsafeBytesFromPacketStream (labels1 !!) -< muxedDfs
 where
  bytesToPacketStreams = bundleC $ bytesToPacketStream (== newline) <$> indicesI @n
  timeouts = bundleC $ timeout sTimeout <$> breakupM2Ss
...
unsafeBytesFromPacketStream f =
  packetizerC (const ()) f |> unsafePacketStreamToDf
 where
...
-- | What it says on the tin. Maybe we should upstream this to @clash-protocols@?
bundleC :: forall n a b. Vec n (Circuit a b) -> Circuit (Vec n a) (Vec n b)
bundleC circuits = Circuit (unzip . zipWith ($) (map toSignals circuits) . uncurry zip)
