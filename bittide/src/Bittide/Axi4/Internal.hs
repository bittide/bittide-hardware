...
axiMapC ::
  ...
  (Axi4StreamM2S conf userTypeA -> Axi4StreamM2S conf userTypeB) ->
  Circuit
   (Axi4Stream dom conf userTypeA) (Axi4Stream dom conf userTypeB)
axiMapC f = Circuit $ \(fwd, bwd) -> (bwd, fmap (fmap f) fwd)
