...
si539xSpiDriverMM minTargetPs =
  circuit $ \((mm, wb), miso) -> do
    ...
    [wb0, wb1, wb2, wb3, wb4, wb5] <- deviceWbC "Si539xSpi" -< (mm, wb)

    (Fwd regOp, _regOpActivity) <- registerWbCI regOpConfig defRegOp -< (wb0, Fwd noWrite)
    (_a1, Fwd commitActivity) <- registerWbCI commitConfig () -< (wb1, Fwd noWrite)
    registerWbCI_ busyConfig False -< (wb2, Fwd (Just <$> busy))
    registerWbCI_ readDataConfig 0 -< (wb3, Fwd readData)
    (spiDone, _spiDoneActivity) <- registerWbCI spiDoneConfig False -< (wb4, Fwd noWrite)
    (_a5, Fwd resetActivity) <- registerWbCI resetSpiConfig () -< (wb5, Fwd noWrite)
 ...
 where
  ...
  isBusWrite (BusRead _) = False
  isBusWrite BusIdle = False
