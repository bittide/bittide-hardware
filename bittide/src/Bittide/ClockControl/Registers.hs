...
clockControlWb margin frameSize linkMask linksOk (bundle -> counters) = circuit $ \(mm, wb) -> do
  ...
  registerWbCI_ linksStableConfig 0 -< (wbLinksStable, Fwd linksStableWrite)
  registerWbCI_ linksSettledConfig 0 -< (wbLinksSettled, Fwd linksSettledWrite)
 ...
 where
  ...
  linksStableConfig = (registerConfig "links_stable"){access = ReadOnly}
  linksSettledConfig = (registerConfig "links_settled"){access = ReadOnly}
