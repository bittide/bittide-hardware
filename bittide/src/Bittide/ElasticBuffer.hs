...
provides monitoring capabilities. The primary use case is to enable the clock control
CPU to perform buffer initialization during the system startup phase.
...
xilinxElasticBufferWb clkRead rstRead SNat clkWrite wdata = circuit $ \wb -> do
  [wbCommand, wbDataCount, wbUnderflow, wbOverflow] <- deviceWb "ElasticBuffer" -< wb

  -- Command register: write-only, adds or removes a single element from the buffer occupancy
  ...
  -- Synchronize overflow pulse from write domain to read domain using xpmCdcPulse
