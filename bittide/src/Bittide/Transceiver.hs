...
transceiverPrbsN opts inputs = outputs
 where
  ...
  tOutput =
    transceiverPrbs opts
      <$> zipWith (\tInput tInputFromH -> tInput{fromHandshake = tInputFromH}) tInputVec toTransceiver
  toHandshake = (.toHandshake) <$> tOutput
  hOutput =
    userDataHandshake
      <$> zipWith (\hInput hInputFromT -> hInput{fromTransceiver = hInputFromT}) hInputVec toHandshake
  toTransceiver = (.toTransceiver) <$> hOutput
