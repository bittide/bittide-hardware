...
transceiverPrbsN opts inputs@Inputs{clock, reset, refClock} =
 ...
 where
  ...
  outputs =
    go (txClk1s, txClock, txClkActives)
      ...
      <*> inputs.rxReadys
      <*> rxClockNws
      <*> repeat (pure 0 :: Signal tx (Unsigned 8))
      <*> repeat (pure 0 :: Signal tx (Unsigned 8))
  ...
  (txClk1s, txClock, txClkActives) = Gth.xilinxGthUserClockNetworkTx @tx @tx txOutClk txUsrClkRst
  ...
  go (clockTx1, clockTx2, txActive) transceiverIndex channelName clockPath rxSim rxN rxP channelReset txData txStart rxReady (clockRx1, clockRx2, rxActive) _unused1 _unused2 =
    transceiverPrbs
      opts
      Input
        { channelName
