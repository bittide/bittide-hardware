...
--  5. Wait for 'TransceiverInput.txReady'
...
data TransceiverOutputs n tx rx txS free = TransceiverOutputs
  ...
  -- ^ See 'TransceiverOutput.txClock'
  , txResets :: Vec n (Reset tx)
  -- ^ See 'TransceiverOutput.txReset'
  , txReadys :: Vec n (Signal tx Bool)
  -- ^ See 'TransceiverOutput.txReady'
  , txSamplings :: Vec n (Signal tx Bool)
  -- ^ See 'TransceiverOutput.txSampling'

  , txPs :: Vec n (Signal txS (BitVector 1))
  -- ^ See 'TransceiverOutput.txP'
  , txNs :: Vec n (Signal txS (BitVector 1))
  -- ^ See 'TransceiverOutput.txN'

  , rxClocks :: Vec n (Clock rx)
  -- ^ See 'TransceiverOutput.rxClock'
  , rxResets :: Vec n (Reset rx)
  -- ^ See 'TransceiverOutput.rxReset'
  , rxDatas :: Vec n (Signal rx (Maybe (BitVector 64)))
  -- ^ See 'TransceiverOutput.rxData'

  , linkUps :: Vec n (Signal free Bool)
  -- ^ See 'TransceiverOutput.linkUp'
  , linkReadys :: Vec n (Signal free Bool)
  -- ^ See 'TransceiverOutput.linkReady'
  , stats :: Vec n (Signal free ResetManager.Statistics)
  -- ^ See 'TransceiverOutput.stats'
  }

data TransceiverOutput tx rx txS free = TransceiverOutput
  { txClock :: Clock tx
  ...
  -- 'TransceiverInput.txReady' to be asserted before starting to send 'txData'.
  , txSampling :: Signal tx Bool
  -- ^ Data is sampled from 'TransceiverInput.txSampling'
...
data TransceiverInput tx rx ref free rxS = TransceiverInput
  { clock :: Clock free
  -- ^ Any "always on" clock
  , reset :: Reset free
  -- ^ Reset signal for the entire transceiver
  , referenceClock :: Clock ref
  ...
  -- 'TransceiverOutput.txSamplingOnNext' is asserted. Is sampled when
  -- 'TransceiverOutput.txData' is asserted.
  , txReady :: Signal tx Bool
  -- ^ When asserted, signal to neighbor that next word will be user data. This
  -- signal is ignored until 'TransceiverOutput.txReady' is asserted. Can be tied
  -- to 'True'.
...
data TransceiverInputs tx rx ref free rxS n = TransceiverInputs
  { clock :: Clock free
  -- ^ See 'TransceiverInput.clock'
  , reset :: Reset free
  -- ^ See 'TransceiverInput.reset'
  , referenceClock :: Clock ref
  -- ^ See 'TransceiverInput.referenceClock'
  , channels :: Vec n String
  -- ^ See 'TransceiverInput.channel'
  , clockPaths :: Vec n String
  -- ^ See 'TransceiverInput.clockPath'
  , rxNs :: Vec n (Signal rxS (BitVector 1))
  -- ^ See 'TransceiverInput.rxN'
  , rxPs :: Vec n (Signal rxS (BitVector 1))
  -- ^ See 'TransceiverInput.rxP'
  , txDatas :: Vec n (Signal tx (BitVector 64))
  -- ^ See 'TransceiverInput.txData'
  , txReadys :: Vec n (Signal tx Bool)
  -- ^ See 'TransceiverInput.txReady'
  , rxReadys :: Vec n (Signal rx Bool)
  -- ^ See 'TransceiverInput.rxReady'
  }
...
transceiverPrbsN ::
  ...
  TransceiverOptions free ->
  TransceiverInputs tx rx ref free rxS n ->
  TransceiverOutputs n tx rx txS free
transceiverPrbsN opts inputs@TransceiverInputs{clock, reset, referenceClock} = TransceiverOutputs
  -- tx
 ...
 where
  ...
  go transceiverIndex channel clockPath rxN rxP txData txReady rxReady =
    transceiverPrbs opts TransceiverInput
      { channel, clockPath, rxN, rxP, txData, txReady, rxReady, transceiverIndex
      , clock, reset, referenceClock
      }
...
transceiverPrbs ::
  ...
  TransceiverOptions free ->
  TransceiverInput tx rx ref free rxS ->
  TransceiverOutput tx rx txS free
transceiverPrbs opts args@TransceiverInput{clock, reset} =
 ...
 where
  ...
  ( txN, txP, txClock, rxClock, rx_data0, reset_tx_done, reset_rx_done, tx_active
   , rxCtrl0, rxCtrl1, rxCtrl2, rxCtrl3 )
    = gthCore
        ...
        txctrl
        clock -- drpclk_in
        args.referenceClock -- gtrefclk0_in
  ...
  alignedRxData0 = withClock rxClock $
    WordAlign.alignBytesFromMsbs @8 rxUserData rx_data0
