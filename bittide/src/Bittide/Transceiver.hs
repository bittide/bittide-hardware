...
data Input tx rx tx1 rx1 ref free rxS = Input
  ...
  is asserted.
  -}
  , drpAddr :: Signal free (BitVector 9)
  -- ^ DRP address driven to the GTH channel. See "Bittide.Transceiver.Drp".
  , drpDi :: Signal free (BitVector 16)
  -- ^ DRP write data.
  , drpEn :: Signal free (BitVector 1)
  -- ^ DRP enable (pulse for one cycle to start a read/write).
  , drpWe :: Signal free (BitVector 1)
  -- ^ DRP write enable (assert with 'drpEn' for a write).
  }

{- | Per-channel DRP inputs in the @free@ (@drpclk@) domain:
@(drpaddr, drpdi, drpen, drpwe)@. See "Bittide.Transceiver.Drp".
-}
type DrpIn free =
  ( Signal free (BitVector 9)
  , Signal free (BitVector 16)
  , Signal free (BitVector 1)
  , Signal free (BitVector 1)
  )
...
data Inputs tx rx ref free rxS n = Inputs
  ...
  -- ^ See 'Input.txData'
  , drps :: Vec n (DrpIn free)
  -- ^ Per-channel DRP inputs. See 'DrpIn' and "Bittide.Transceiver.Drp".
  }
...
data Output tx rx tx1 rx1 txS free = Output
  ...
  , drpDo :: Signal free (BitVector 16)
  -- ^ DRP read data from the GTH channel (@drpdo_out@).
  , drpRdy :: Signal free (BitVector 1)
  -- ^ DRP ready/valid from the GTH channel (@drprdy_out@).
  }
...
data Outputs n tx rx txS free = Outputs
  ...
  -- ^ See 'Output.stats'
  , drpDos :: Vec n (Signal free (BitVector 16))
  -- ^ See 'Output.drpDo'
  , drpRdys :: Vec n (Signal free (BitVector 1))
  -- ^ See 'Output.drpRdy'
  }
...
outputsToCOutputs outputs =
  COutputs
    ...
    , stats = outputs.stats
    , drpDos = outputs.drpDos
    , drpRdys = outputs.drpRdys
    }
...
transceiverPrbsN opts inputs@Inputs{clock, reset, refClock} =
  Outputs
    ...
    , drpDos = map (.drpDo) outputs
    , drpRdys = map (.drpRdy) outputs
    }
 where
  ...
  go (clockTx1, clockTx2, txActive) channelName clockPath rxSim rxN rxP channelReset txData (drpAddr, drpDi, drpEn, drpWe) (clockRx1, clockRx2, rxActive) =
    transceiverPrbs
      opts
      Input
        { channelName
