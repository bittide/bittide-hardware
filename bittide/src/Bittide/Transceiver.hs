...
data TransceiverInputFirstHalf tx rx tx1 rx1 ref free rxS = TransceiverInputFirstHalf
  { clock :: Clock free
  , reset :: Reset free
  , channelReset :: Reset free
  , refClock :: Clock ref
...
data TransceiverInputSecondHalf tx rx tx1 rx1 ref free rxS = TransceiverInputSecondHalf
  { rxActive :: Signal rx (BitVector 1)
  , transceiverIndex :: Unsigned 3
  , channelName :: String
  , clockPath :: String
...
data TransceiverOutputs n tx rx txS free = TransceiverOutputs
  { txClock :: Clock tx
   ...
   where
    ...
    tInputs =
      TransceiverInputs
        ...
        , rxNs
        , rxPs
        , fromHandshakes = toTransceiver
        }
    tOutputs@TransceiverOutputs{txPs, txNs, txSims} = transceiverPrbsN config tInputs
    toHandshake = tOutputs.toHandshakes

    hInputs =
      HandshakeInputs
        { clock
        , reset
        , txClock = tOutputs.txClock
        , rxClocks = tOutputs.rxClocks
        , txDatas = cInputs.txDatas
        , txStarts = cInputs.txStarts
        , rxReadys = cInputs.rxReadys
        , fromTransceivers = toHandshake
        }
    hOutputs =
      userDataHandshakeN
        hInputs
    toTransceiver = hOutputs.toTransceivers

    outputs =
      Outputs
        { -- tx
          txClock = tOutputs.txClock
        , txReset = tOutputs.txReset
        , txReadys = hOutputs.neighborReceiveReadyTxs
        , txSamplings = hOutputs.txIsUserDatas
        , handshakesDoneTx = tOutputs.handshakesDoneTx
        , txSims = tOutputs.txSims
        , -- rx
          rxClocks = tOutputs.rxClocks
        , rxResets = tOutputs.rxResets
        , rxDatas = hOutputs.wordToUsers
        , handshakesDone = tOutputs.handshakesDone
        , -- transceiver
          txPs = tOutputs.txPs
        , txNs = tOutputs.txNs
        , -- free
          debugLinkUps = hOutputs.debugLinkUps
        , debugLinkReadys = hOutputs.debugLinkReadys
        , handshakesDoneFree = tOutputs.handshakesDoneFree
        , neighborReceiveReadys = hOutputs.neighborReceiveReadys
        , neighborTransmitReadys = hOutputs.neighborTransmitReadys
        , stats = tOutputs.stats
        }
...
transceiverPrbsN ::
  ...
  Config free ->
  TransceiverInputs tx rx ref free rxS n ->
  TransceiverOutputs n tx rx txS free
transceiverPrbsN opts inputs = outputs
 where
  ...
  tInputSecondVec =
    TransceiverInputSecondHalf
      ...
      <*> inputs.fromHandshakes

  tInputVec = halvesToWhole <$> tInputFirstVec <*> tInputSecondVec

  -- halvesToWhole :: TransceiverInputFirstHalf -> TransceiverInputSecondHalf -> TransceiverInput
  halvesToWhole firstHalf secondHalf = output
   where
    output =
      TransceiverInput
        { clock = firstHalf.clock
        , reset = firstHalf.reset
        , channelReset = firstHalf.channelReset
        , refClock = firstHalf.refClock
        , clockTx1 = firstHalf.clockTx1
        , clockTx2 = firstHalf.clockTx2
        , txActive = firstHalf.txActive
        , clockRx1 = firstHalf.clockRx1
        , clockRx2 = firstHalf.clockRx2
        , rxActive = secondHalf.rxActive
        , transceiverIndex = secondHalf.transceiverIndex
        , channelName = secondHalf.channelName
        , clockPath = secondHalf.clockPath
        , rxSim = secondHalf.rxSim
        , rxN = secondHalf.rxN
        , rxP = secondHalf.rxP
        , fromHandshake = secondHalf.fromHandshake
        }
  ...
  outputVec = transceiverPrbs opts <$> tInputVec

  outputs =
    TransceiverOutputs
      { txClock = txOutClk
      , txReset = txReset
      , handshakesDoneTx = (.prbsHandshakeDoneTx) <$> outputVec
      ...
      , rxClocks = (.rxOutClock) <$> outputVec
      , rxResets = (.rxReset) <$> outputVec
      , handshakesDone = (.prbsHandshakeDone) <$> outputVec
      , handshakesDoneFree = (.prbsHandshakeDoneFree) <$> outputVec
      , stats = (.stats) <$> outputVec
      , toHandshakes = (.toHandshake) <$> outputVec
      }
