...
gppeC cfg@GppeConfig{} =
  circuit $ \(mm, Fwd linkIn, (scatterCalMM, scatterCalWb), (gatherCalMM, gatherCalWb), jtag) -> do
    ...
    constBwd cfg.uartPrefix -< uartPfx
    ...
    linkOut <-
      gatherUnitWbC cfg.gatherConfig -< ((gatherMM, gatherWb), (gatherCalMM, gatherCalWb))
...
managementUnitC
  ( ManagementConfig
    ...
    idleSink <| vecCircuits (constBwd <$> captureUgnPrefixes) -< captureUgnPfxs

    (scatterPfxs, scatterMMWbs) <- unzipC -< scatterPfxMMWbs
    idleSink <| vecCircuits (constBwd <$> peScatterPrefixes) -< scatterPfxs

    (gatherPfxs, gatherMMWbs) <- unzipC -< gatherPfxMMWbs
    idleSink <| vecCircuits (constBwd <$> peGatherPrefixes) -< gatherPfxs
