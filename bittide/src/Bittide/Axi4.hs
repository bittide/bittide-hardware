   ...
   where
    toDf (waM2S, dfAck) = (waS2M, dfData)
     where
      ...
      dfData = mux (isWriteAddress <$> waM2S) (Just <$> waM2S) (pure Nothing)
      fromAck (Ack b) = b
   ...
   where
    toAxi4 (dfData, waS2M) = (dfAck, waM2S)
     where
      ...
      waM2S = mux (isJust <$> dfData) (fromJust <$> dfData) (pure M2S_NoWriteAddress)
   ...
   where
    toDf (wdM2S, dfAck) = (wdS2M, dfData)
     where
      ...
      dfData = mux (isWriteData <$> wdM2S) (Just <$> wdM2S) (pure Nothing)
      fromAck (Ack b) = b
   ...
   where
    toAxi4 (dfData, wdS2M) = (dfAck, wdM2S)
     where
      ...
      wdM2S = mux (isJust <$> dfData) (fromJust <$> dfData) (pure M2S_NoWriteData)
   ...
   where
    toDf (wrS2M, dfAck) = (wrM2S, dfData)
     where
      ...
      dfData = mux (isWriteResponse <$> wrS2M) (Just <$> wrS2M) (pure Nothing)
      fromAck (Ack b) = b
   ...
   where
    toAxi4 (dfData, wrM2S) = (dfAck, wrS2M)
     where
      ...
      wrS2M = mux (isJust <$> dfData) (fromJust <$> dfData) (pure S2M_NoWriteResponse)
   ...
   where
    toDf (raM2S, dfAck) = (raS2M, dfData)
     where
      ...
      dfData = mux (isReadAddress <$> raM2S) (Just <$> raM2S) (pure Nothing)
      fromAck (Ack b) = b
   ...
   where
    toAxi4 (dfData, raS2M) = (dfAck, raM2S)
     where
      ...
      raM2S = mux (isJust <$> dfData) (fromJust <$> dfData) (pure M2S_NoReadAddress)
   ...
   where
    toDf (rdS2M, dfAck) = (rdM2S, dfData)
     where
      ...
      dfData = mux (isReadData <$> rdS2M) (Just <$> rdS2M) (pure Nothing)
      fromAck (Ack b) = b
   ...
   where
    toAxi4 (dfData, rdM2S) = (dfAck, rdS2M)
     where
      ...
      rdS2M = mux (isJust <$> dfData) (fromJust <$> dfData) (pure S2M_NoReadData)
