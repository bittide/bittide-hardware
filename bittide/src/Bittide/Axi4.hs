...
axisToByteStream ::
  ...
  , Show userType) =>
  Circuit
    (PackedAxi4Stream dom ('Axi4StreamConfig (dataWidth + 1) idWidth destWidth) userType)
    (Axi4Stream dom ('Axi4StreamConfig 1 idWidth destWidth) userType)
axisToByteStream = Circuit (mealyB go Nothing)
 where
  go axiStored (axiIn, Axi4StreamS2M{_tready}) = (axiNext, (bigS2M, rhsM2S))
   where
    axiTmp = concatAxi axiStored Nothing <|> axiIn
    (rhsM2S, axiRest) = splitAxi @1 axiTmp
    axiNext
      | (isJust rhsM2S && _tready) || isNothing rhsM2S = axiRest
      | otherwise = axiStored

    bigS2M = Axi4StreamS2M{_tready = (isJust rhsM2S && _tready) && isNothing axiNext}
...
data BufferState bufferSize = AwaitingData | BufferFull | PacketComplete (Index bufferSize)
  deriving (Generic, NFDataX, Show)
data ReadStateMachine bufferSize =
    Idle | ReadingPacketSize | ReadingPacket (Index bufferSize) | ClearingPacketLength | ClearingStatus
  deriving (Generic, NFDataX, Show)
...
rxReadMasterC ::
  ...
  Circuit
  ()
  (Wishbone dom 'Standard addrWidth (Bytes nBytes), Axi4Stream dom (BasicAxiConfig nBytes) ())
...
rxReadMaster ::
  forall dom nBytes addrWidth bufferDepth .
  (HiddenClockResetEnable dom, 1 <= bufferDepth * nBytes, 1 <= bufferDepth, KnownNat addrWidth, KnownNat nBytes) =>
  SNat bufferDepth ->
  ( Signal dom (WishboneS2M (Bytes nBytes))
  , Signal dom Axi4StreamS2M
  ) ->
  ( Signal dom (WishboneM2S addrWidth nBytes (Bytes nBytes))
  , Signal dom (Maybe (Axi4StreamM2S (BasicAxiConfig nBytes) ()))
  )
    ...
    go
     ...
     where
      ...
      (writeEnable, addr) = case readState of
        Idle                 -> (False, natToNum @((1 + bufferDepth) * nBytes))
        ReadingPacketSize    -> (False, natToNum @(bufferDepth * nBytes))
        ReadingPacket i      -> (False, pack $ resize $ pack i)
        ClearingPacketLength -> (True, natToNum @(bufferDepth * nBytes))
        ClearingStatus       -> (True, natToNum @((1 + bufferDepth) * nBytes))
      ...
      (_tkeep, _tlast) = case (bufState, readState) of
        (PacketComplete s, ReadingPacket i) -> (mkKeep remaining, remaining <= natToNum @nBytes)
         where
          remaining = satSub SatBound s (shiftL (shiftR i 2) 2)
        _ -> (repeat True, False)
      ...
      nextState = if not acknowledge then (bufState, readState) else
        case (readState, bufState) of
          (Idle,_) -> case (packetComplete, bufferFull) of
            ...
            (True, False) -> (AwaitingData, ReadingPacketSize)
            _   -> (AwaitingData, Idle)
           where
            (packetComplete, bufferFull) = unpack $ resize readData
          (ReadingPacketSize,_)
            | packetSize > 0 -> (PacketComplete packetSize, ReadingPacket 0)
            | otherwise -> (bufState, ClearingPacketLength)
             where
              packetSize = unpack $ resize readData
          (ReadingPacket ((==maxBound) -> True), BufferFull) ->
            (bufState, ClearingPacketLength)
          (ReadingPacket i, _)
            | _tready && lastBytes bufState nextReadState -> (bufState, ClearingPacketLength)
            | _tready -> (bufState, ReadingPacket (satAdd SatBound i (natToNum @nBytes)))
...
data AxiPacketFifoState = AxiAccumulating | AxiPacketComplete | AxiPassThrough
  deriving (Generic, NFDataX, Show)
...
axiStreamPacketFifo fifoDepth = Circuit goCircuit
 where
  ...
  goState AxiAccumulating (lhsM2S, fifoOut, fifoReady, _)
    | fifoReady && maybe False _tlast lhsM2S = AxiPacketComplete
    | not fifoReady && isJust fifoOut = AxiPassThrough
    | otherwise = AxiAccumulating
  goState AxiPacketComplete (_, fifoOut, _, rhsS2M)
    | rhsS2M && maybe False _tlast fifoOut = AxiAccumulating
    | otherwise = AxiPacketComplete
