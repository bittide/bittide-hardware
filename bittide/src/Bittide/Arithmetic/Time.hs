...
-- | Rises after the incoming signal has been 'True' for the specified amount of
-- time. The time is specified in milliseconds. Example invocation:
--
-- > trueForTerm @(Milliseconds 1) Proxy someLimit clk rst signal
--
-- Use 'trueFor' if you know the time to wait for at compile time. If not, use
-- 'trueForSteps'.
trueForSteps ::
  forall (stepSize :: Nat) (n :: Nat) (dom :: Domain) .
  (HasCallStack, KnownDomain dom, KnownNat n, KnownNat stepSize) =>
  Proxy stepSize ->
