...
-- size `s` with the given element. Otherwise, return the list as is.
padToSize :: Maybe Int -> a -> [a] -> [a]
padToSize Nothing _ l = l
padToSize (Just s) a l = L.take s (l <> L.repeat a)
...
memBlobsFromElf ::
  ...
  ByteOrder ->
  -- | Optional size to which we should pad the instruction memBlob and data memBlob
  (Maybe Int, Maybe Int) ->
...
memBlobsFromElf byteOrder (iSize, dSize) elfPath maybeDeviceTree = do
  ...
  let
    ...
    iListPadded = padToSize iSize 0 iList
    dListPadded = padToSize dSize 0 dList
