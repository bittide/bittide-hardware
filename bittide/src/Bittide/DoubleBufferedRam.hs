...
-- | The double buffered Ram component is a memory component that atomically separates
-- read and write addresses. 'SelectedBuffer' selects which buffer is written to, while
-- read operations read from the other buffer.
doubleBufferedRam ::
...
-- | Version of 'doubleBufferedRam' with undefined initial contents.
-- The double buffered Ram component is a memory component that atomically separates
-- read and write addresses. 'SelectedBuffer' selects which buffer is written to,
-- while read operations read from the other buffer.
doubleBufferedRamU ::
  forall dom memDepth a .
...
-- | The double buffered Ram component is a memory component that atomically separates
-- read and write addresses. It contains a blockRam per byte and uses the
-- one hot byte select signal to determine which bytes will be overwritten during a
-- write operation. This components writes to one buffer and reads from the other.
-- 'SelectedBuffer' selects which buffer is written to, while read operations read from
-- the other buffer.
