...
resetManager config clk rst tx_init_done rx_init_done rx_data_good =
 ...
 where
  ...
  extractOutput st = case st of
         --             rst_all rst_rx statistics
    StartTx stats   -> (True,   False, stats)
    TxWait  stats _ -> (False,  False, stats)
    StartRx stats   -> (False,  True,  stats)
    RxWait  stats _ -> (False,  False, stats)
    Monitor stats   -> (False,  False, stats)
