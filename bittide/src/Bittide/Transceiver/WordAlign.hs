...
isAlignSymbol :: forall n. (KnownNat n, 1 <= n) => BitVector n -> Bool
isAlignSymbol bv = popCount bv == 1

isFlippedAlignSymbol :: forall n. (KnownNat n, 1 <= n) => BitVector n -> Bool
isFlippedAlignSymbol bv = popCount (complement bv) == 1
...
align ::
  ...
  Signal dom (Vec n a)
align = curry3 (mealy go (defAlignState @n @a) . bundle)
 where
