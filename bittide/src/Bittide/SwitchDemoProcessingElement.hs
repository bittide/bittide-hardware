...
zeroExtendTimesThree = resize . mul (3 :: Index 4)
...
switchDemoPeWb SNat localCounter maybeDna linkIn = circuit $ \bbMm -> do
  [wb0, wb1, wb2, wb3, wb4, wb5] <- deviceWbI (deviceConfig "SwitchDemoPE") -< bbMm
  Fwd (readStart, _) <- registerWbI readStartCfg 0 -< (wb0, Fwd (pure Nothing))
  Fwd (readCycles, _) <- registerWbI readCyclesCfg 0 -< (wb1, Fwd (pure Nothing))
  Fwd (writeStart, _) <- registerWbI writeStartCfg 0 -< (wb2, Fwd (pure Nothing))
  Fwd (writeCycles, _) <- registerWbI writeCyclesCfg 0 -< (wb3, Fwd (pure Nothing))
  registerWbI_ localClockCycleCounterCfg 0 -< (wb4, Fwd (Just <$> localCounter))
  registerWbI_ bufferCfg (repeat 0) -< (wb5, Fwd (Just <$> buffer))
