...
This way, the MU does not need to inspect every element in the RB for the clock cycle, it just needs to inspect the one entry it knows the clock cycle will eventually be in. For more detail, see the [Ring Buffer alignment](ring-buffer-alignment.html) section.
