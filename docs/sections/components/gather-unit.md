...
The Gather Unit exposes the following registers via the Wishbone interface:

| Name | Access | Description |
|---|---|---|
| `gatherMemory` | Write-Only | The memory buffer where data to be gathered is written. The size depends on the configuration. |
| `metacycleCount` | Read-Only | A 32-bit counter that increments at the end of each metacycle. |
| `metacycleRegister` | Read-Only | **Stalling Register**. Reading from this register stalls the Wishbone bus until the end of the current metacycle. This is used to synchronize software with the hardware schedule. |
