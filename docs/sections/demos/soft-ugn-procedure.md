...
Events are scheduled deterministically to ensure that all possible timing interactions are explored.
*   Initially, one **SEND** event is scheduled far enough into the future for each port, evenly spaced.
*   Initially, one **RECEIVE** event is scheduled far enough into the future for each port, evenly spaced.
...
*   `receive_period / memory_size` and `send_period / memory_size` are coprime to make sure we cover all possible overlaps between ports. We divide by `memory_size` because we know exactly where to write and read messages in our memories.
...
3.  **UGN Calculation**:
    ...
    *   By exchanging timestamps, the system solves for the constant offset (propagation delay) while accounting for the metacycle phase differences.

## Hardware dependency

The protocol strictly depends on the underlying hardware abstraction:
*   **Gather Units**: Used to transmit data. In ringbuffer mode, writing to memory offset $X$ means the data will be sent on the link at metacycle time $X$.
*   **Scatter Units**: Used to receive data. Reading from memory offset $Y$ allows the software to see what was received on the link at metacycle time $Y$.
*   **Metacycle Synchronization**: All actions are referenced to the local metacycle counter, which provides the common timebase for the ringbuffer indices.
